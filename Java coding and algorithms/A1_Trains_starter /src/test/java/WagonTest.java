...
public class WagonTest {
    ...
    //Additional tests Yazan

    @Test
    public void At1_ReversingTheLastWagonInASequenceDoesNotChangeAnything() {
        // Create a sequence of wagons
        passengerWagon1.attachTail(passengerWagon2);
        passengerWagon2.attachTail(passengerWagon3);
        passengerWagon3.attachTail(passengerWagon4);

        // Reverse the last wagon in the sequence
        passengerWagon4.reverseSequence();

        // Check if the state remains unchanged
        assertEquals(passengerWagon1.getLastWagonAttached(), passengerWagon4, "Reversing the last wagon in the sequence should not change anything");
    }

    @Test
    public void At2_AttachingTailToWagonWithExistingTailThrowsException() {
        // Create a sequence of two wagons
        passengerWagon1.attachTail(passengerWagon2);

        // Attempt to attach a tail to passengerWagon1, which already has a tail
        Throwable t = assertThrows(IllegalStateException.class,
                () -> passengerWagon1.attachTail(new PassengerWagon(8005, 20)));

        assertTrue(t.getMessage().contains(passengerWagon1.toString().replaceAll("[\\[\\]]","")),
                "Exception message should report that " + passengerWagon1.toString() + " is already pulling a wagon.");
    }

    @Test
    public void At3_ReversingEmptySequenceDoesNotAffectAnything() {
        // Attempt to reverse an empty sequence
        Wagon result = passengerWagon1.reverseSequence();

        assertNull(result, "Reversing an empty sequence should return null.");

        // Check if the state remains unchanged
        assertFalse(passengerWagon1.hasNextWagon(), "Reversing an empty sequence should not affect the wagon.");
        assertFalse(passengerWagon1.hasPreviousWagon(), "Reversing an empty sequence should not affect the wagon.");
    }
