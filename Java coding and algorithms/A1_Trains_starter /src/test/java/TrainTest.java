...
public class TrainTest {
    ...
    public void setup() {
        ...
        passengerWagon8002.attachTail((Wagon) (Object) new PassengerWagon(8003, 18));
    ...
    // Additional tests Danny

    @Test
    public void TD1_MoveOneWagonShouldRespectRepresentationInvariant() {
        assertTrue(passengerTrain.moveOneWagon(8002, trainWithoutWagons),
                "can move one wagon from mid-sequence to an empty train");
        assertEquals(passengerTrain.findWagonById(8001).getNextWagon(), passengerTrain.findWagonById(8003));
        assertEquals(passengerTrain.findWagonById(8003).getPreviousWagon(), passengerTrain.findWagonById(8001));
        assertSame(passengerWagon8002, trainWithoutWagons.getFirstWagon());
    }

    @Test
    public void TD2_MoveOneWagonShouldNotMoveToAFullTrain() {
        //Fill other train with 7 wagons
        for (int i = 0; i < 8; i++) {
            passengerTrain.moveOneWagon(8000 + i, trainWithoutWagons);
        }

        passengerWagon1.getNextWagon().detachFront();
        passengerWagon1.detachTail();
        passengerTrain.insertAtFront(passengerWagon1);
        assertFalse(passengerTrain.moveOneWagon(8011, trainWithoutWagons), "should not be able to move one wagon to a full train");

    }

    @Test
    public void TD3_ShouldReturnFalseWhenMovingNonExistingWagonInSequence() {
        assertFalse(passengerTrain.moveOneWagon(8011, trainWithoutWagons));
